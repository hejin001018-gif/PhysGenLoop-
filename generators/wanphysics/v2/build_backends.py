...
def build_v2_runner(
    *,
    ...
    python = paths.get("envs", {}).get("main")
    vllm_python = paths.get("envs", {}).get("vllm")
    eval_step = paths.get("eval_step")

    artifacts = RunArtifacts(run_dir)

    generator = WanSubprocessGenerator(
        python=python,
        model_path=str(Path(cfg.get("paths", {}).get("root", "/root/PhysGenLoop-")) / "models/wan2.2_ti2v_5b") if paths.get("root") else "/root/PhysGenLoop-/models/wan2.2_ti2v_5b",
    ...
    vllm_lifecycle = Sam2VlmSubprocessCritic(
        ...
        vllm_model=paths.get("models", {}).get("vllm", "/root/PhysGenLoop-/models/Qwen3-VL-8B-Instruct"),
    ...
    if scorers_cfg.get("semantic_enabled", False):
        ...
        try:
            ...
            env = dotenv_values(str(Path(cfg.get("paths", {}).get("root", "/root/PhysGenLoop-")) / ".env"))
        except Exception:  # noqa: BLE001
            env = {}
    ...
    ckpt_root = ckpt_root or "/root/PhysGenLoop-/checkpoints/repair_agent/repair-agent-v3.1-proxy-20260717"
    ...
    editor_cls = StrictProPainterLocalEditor if bool(local_cfg.get("strict_manifest", True)) else ProPainterLocalEditor
    editor = editor_cls(
        propainter_repo=local_cfg.get("propainter_repo", "/root/PhysGenLoop-/models/ProPainter"),
        python=python,
        output_root=sample_dir,
