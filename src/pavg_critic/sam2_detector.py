    ...
    def _track_with_sam2(
        ...
        with TemporaryDirectory(prefix="pavg_sam2_") as temporary:
            ...
            for obj_id, obj in enumerate(objects):
                ...
                if not math.isfinite(x_pct) or not math.isfinite(y_pct):
                    raise ValueError("SAM2 object seed percentages must be finite")
                x_pct = min(100.0, max(0.0, x_pct))
                y_pct = min(100.0, max(0.0, y_pct))
                self._object_names[obj_id] = name
                x_px = min(
                    x_pct / 100.0 * self._width,
                    float(max(0, self._width - 1)),
                )
                y_px = min(
                    y_pct / 100.0 * self._height,
                    float(max(0, self._height - 1)),
                )
