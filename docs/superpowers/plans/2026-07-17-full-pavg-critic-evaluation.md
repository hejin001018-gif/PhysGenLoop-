...
### E12 — GPU scheduling measurement and deferred recovery
...
### E17 — second A100 integration and cloud2 owner archive

- Timestamp: `2026-07-19T21:10:39+08:00`; the new server `px-cloud1.matpool.com:27339` passed read-only audit with Ubuntu 22.04, 12 CPU, 90 GiB RAM, 200 GiB disk, and one idle NVIDIA A100-PCIE-40GB (driver `570.86.10`). Key-only SSH was configured using the existing dedicated ED25519 key; the supplied password was not stored in the repository or runtime artifacts.
- The new server cloned source commit `838be8c`, created Python 3.12.13, installed Torch `2.8.0+cu128`, vLLM `0.11.0`, Transformers `4.57.0`, ModelScope `1.38.1`, and the project package. Qwen3-VL-8B-Instruct was downloaded from ModelScope; vLLM passed HTTP 200 with the same deterministic 16-frame policy, `gpu_memory_utilization=0.66`, and `max_num_seqs=2`.
- The fixed even manifest was retained unchanged. All `833/833` videos were materialized after preserving literal `+` characters in S3 paths; 4,644 observation files were synchronized, and no sample was removed after initial download failures.
- cloud2 owner reached terminal counts `1,731/1,731` for D1, M4 and M5. Its automatic merger validated `5,193` unique predictions, `3,462` M4/M5 diagnostics (D1 intentionally has no diagnostics), exact owner manifest binding and observation metadata, then created remote archive SHA-256 `879064c0b12a2ef48a071bc9318bdc7715404ea51251a28aa57a5817e2a8eaf4`.
- The owner archive was downloaded to `outputs/benchmarks/videophy2-full-pavg-qwen3vl8b/cloud2-owner/`; local SHA, key uniqueness, archive path safety and secret scan all passed. The old cloud2 evaluator is finished and only its idle vLLM remains; the new cloud1 server continues the even evaluation.
- At this checkpoint the new even run had D1 `768/833`, M4 `262/590` and M5 `219/590` (valid observation subset); the remaining 239 no-observation rows are reserved for the next SAM2 recovery stage. No prompt, model, frame policy, threshold or weight was changed.
